...
$stmt = $pdo->query('SELECT id, nom FROM types ORDER BY id DESC');
$types = $stmt->fetchAll();
?>
<!DOCTYPE html>
<html lang="fr">
...
<body class="bg-gray-900 text-white min-h-screen bg-[radial-gradient(ellipse_at_top,_rgba(160,32,240,0.18),_transparent_55%),radial-gradient(ellipse_at_bottom,_rgba(255,138,51,0.12),_transparent_50%)]">
    ...
    <main class="max-w-5xl mx-auto px-6 py-12">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <a href="type-creation.php" class="rounded-xl px-5 py-2.5 text-sm font-semibold text-white bg-[linear-gradient(90deg,#A020F0,#D94CF0,#FF5E9D,#FF8A33)] shadow-md hover:opacity-90 transition duration-200">Ajouter un type</a>
        </div>
                ...
                <tbody class="divide-y divide-white/10 text-gray-200">
                    <?php foreach ($types as $type): ?>
                        <tr class="hover:bg-white/5 transition duration-200">
                            <td class="px-4 py-4 font-semibold text-white"><?= htmlspecialchars((string) $type['id']) ?></td>
                            <td class="px-4 py-4"><?= htmlspecialchars($type['nom']) ?></td>
                            <td class="px-4 py-4">
                                <div class="flex flex-wrap gap-2">
                                    <a href="type-modification.php?id=<?= htmlspecialchars((string) $type['id']) ?>" class="px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-[linear-gradient(90deg,#A020F0,#D94CF0,#FF5E9D,#FF8A33)] shadow-md hover:opacity-90 transition duration-200">Modifier</a>
                                    <a href="type-suppression.php?id=<?= htmlspecialchars((string) $type['id']) ?>" class="px-3 py-1.5 rounded-lg text-xs font-semibold text-red-200 bg-red-500/20 hover:bg-red-500/30 transition duration-200" onclick="return confirm('Supprimer ce type ?')">Supprimer</a>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
