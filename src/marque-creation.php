...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nom !== '' && $adresse !== '') {
        $stmt = $pdo->prepare('INSERT INTO marques (nom, adresse) VALUES (:nom, :adresse)');
...
<head>
    ...
    <title>Ajouter une marque - Influmatch</title>
...
<body class="bg-gray-900 text-white min-h-screen bg-[radial-gradient(ellipse_at_top,_rgba(160,32,240,0.18),_transparent_55%),radial-gradient(ellipse_at_bottom,_rgba(255,138,51,0.12),_transparent_50%)]">
    ...
    <main class="max-w-xl mx-auto px-6 py-12">
        <h1 class="text-3xl font-bold">Ajouter une marque</h1>
        ...
        <form method="post" class="mt-6 bg-gray-800/60 border border-white/10 rounded-2xl shadow-lg p-6 space-y-4">
            <div>
                ...
                <input type="text" id="nom" name="nom" required class="w-full bg-gray-900/60 border border-white/10 rounded-xl px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#D94CF0]/60">
            </div>
            <div>
                <label class="block text-sm text-white/80 mb-2" for="adresse">Adresse</label>
                <input type="text" id="adresse" name="adresse" required class="w-full bg-gray-900/60 border border-white/10 rounded-xl px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#D94CF0]/60">
            </div>
